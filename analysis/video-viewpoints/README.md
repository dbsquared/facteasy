...
| `extract_quote.py` | 从 Whisper 逐字稿按时间区间抽取每条观点的原文（`quote.segments`）；**按时间戳匹配，幂等保留已补正文本与关键句** |
| `_fold_fix.py` | 勘误落库迁移：把每段 `fix` 落进 `text` 并删除 `fix`（一次性迁移，已执行） |
...
- **核查面板**：右侧三页签，随播放进度自动切换当前观点
  - `原文逐字稿` — 关键句高亮；ASR 同音讹误已按语义与史实补正（直接落进正文）
...
`credibility.md` 对每个观点按统一模板给出：**原文逐字稿**（关键句高亮，ASR 同音讹误已按语义与史实补正）→ **来源性质判定**（`转述` / `专属` / `原创` 三标签，可并存）→ **出处与根据**（编号 S1… + 类型 + 权威层级 T0–T4 + 核验状态）→ 可靠程度 / 时效性 / 适用范围 → 关联强度 → 成立环节 → 疑点与证据不足（每条论点标注 `根据` 与引用编号）→ 综合评级（A/B/C/D/N-A）。
